...
# +
function (m::Matsuno)(h, u)


    Threads.@threads for i in eachindex(m.hnew)
        @inbounds m.hnew[i] = m.Γ[i] * h[i]
    end

    ldiv!(m.unew, m.Px, m.hnew )

    Threads.@threads for i in eachindex(m.hnew)
        @inbounds m.hnew[i] = m.Dx[i] * h[i]
    end
    ...
    Threads.@threads for i in eachindex(m.H)
        @inbounds h[i]    = m.H[i] * u[i]
        @inbounds m.I₀[i] = m.Γ[i] * u[i]
    end
    ...
    Threads.@threads for i in eachindex(m.unew)
        @inbounds h[i] -= (m.I₃[i] * m.Dx[i] + m.hnew[i] * m.H[i]) * m.ϵ * m.Π⅔[i]
    ...
    Threads.@threads for i in eachindex(m.unew)
        @inbounds u[i]  =  m.I₁[i] - m.unew[i] * m.Dx[i] * m.ϵ/2 * m.Π⅔[i]
    end 
