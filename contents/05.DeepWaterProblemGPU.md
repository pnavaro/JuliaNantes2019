...
We reuses data structures from the CPU version of @sec:deep but reimplements the computation loops for GPU execution.
...
- `CuArray`: GPU array type `CuArray` that works like regular Julia arrays but live in GPU memory
- `CUFFT`: CUDA-accelerated FFT library (much faster than CPU FFTW)
