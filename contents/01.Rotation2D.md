...
This document aims to explain how to model a rotation in a two-dimensional domain using an advection equation. We will explore the partial differential equation governing this phenomenon and show how to solve it numerically using the Julia programming language.


The advection equation for a rotation in two dimensions is given by:

$$
\frac{d f}{dt} +  (y \frac{d f}{dx} - x \frac{d f}{dy}) = 0
$$

This equation describes how a scalar quantity, denoted by the function $f$, evolves over time under the influence of a rotational velocity field.
...
- $\frac{df}{dt}$ : Time variation of the function $f$.
- $y \frac{d f}{dx} - x \frac{d f}{dy}$ : Transport term due to rotation.

## Implementation in Julia

To solve this equation numerically, we will use the Julia programming language. Here are the necessary libraries for our implementation:
...
- **FFTW** : Used for fast Fourier transforms, essential for efficiently solving partial differential equations.
- **LinearAlgebra** : Provides tools for linear algebraic operations.
- **Plots** : Allows visualizing results as graphs.
- **BenchmarkTools** : Used to measure code performance.
...
In summary, we have explored how to model a rotation in a two-dimensional
domain using an advection equation. We have seen how to solve this
equation numerically using Julia and visualize the results. Here come the steps to optimatize the computation:
...
Potential applications of this work include modeling physical phenomena involving rotations, such as fluid flows or atmospheric movements.
